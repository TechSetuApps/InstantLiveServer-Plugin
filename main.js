...
const AMAZON_LINK = 'https://github.com/TechSetuApps/InstantLiveServer/releases/tag/v1.4.0';
...
// ACode ka root container dhundo
function getAcodeRoot() {
  return (
...
// #quick-tools ka top position dhundho
// Panel ka bottom = quicktools ka top
function getQuickToolsOffset() {
  const qt = document.querySelector('#quick-tools');
  if (!qt) return 0;
...
// Panel ko body mein add karo
function appendToAcode(el) {
  document.body.appendChild(el);
}

// Panel bottom = quicktools top pe exactly fit karo
function adjustPanelBottom() {
  if (!panel || isFs || resizing) return; // resize ke dauran override mat karo!
  const qt = document.querySelector('#quick-tools');
  if (!qt) return;
  const rect = qt.getBoundingClientRect();
...
function applyViewportPosition() {
  ...
  // Portrait me keyboard ke liye kuch nahi karo
  // adjustPanelBottom sirf panel open pe ek baar call hoga
}
...
function showError() {
  ...
  d.id = 'iw-err';
  d.style.cssText = `position:absolute;left:0;right:0;bottom:0;top:0;background:#18181b;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:28px;text-align:center;z-index:5;`;
  d.innerHTML = `
    <div style="width:68px;height:68px;background:linear-gradient(135deg,#2563eb,#1d4ed8);border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:34px;font-weight:900;color:#fff;margin-bottom:20px;box-shadow:0 8px 28px rgba(255,255,255,0.08);">W</div>
    <p style="color:#e2e8f0;font-size:15px;font-weight:700;margin-bottom:8px;">InstantLive Server is not running</p>
    <p style="color:#64748b;font-size:12px;margin-bottom:22px;line-height:1.7;max-width:230px;">Open InstantLive Server, select an HTML file and tap Start Hosting.</p>
    <a href="${AMAZON_LINK}" style="display:flex;align-items:center;gap:8px;background:#2563eb;color:#fff;padding:11px 22px;border-radius:11px;text-decoration:none;font-size:13px;font-weight:700;margin-bottom:12px;">${ic.dl} Download InstantLive Server</a>
...
function toggleFullscreen() {
  ...
  if (isFs) {
    ...
    panel.style.cssText = `position:fixed;left:0;top:0;width:100%;height:100%;background:#18181b;z-index:9999;display:flex;flex-direction:column;`;
  } else {
    // Fullscreen exit — poora panel rebuild karo fresh orientation ke saath
    // Taaki resize handle landscape → portrait switch pe sahi rahe
...
function addResizeHandle(p, isLand) {
  // Outer invisible touch target — bada touch area
  const h = document.createElement('div');
  h.style.cssText = `position:absolute;background:transparent;z-index:51;${isLand?'left:0;top:50%;transform:translateY(-50%);width:24px;height:80px;cursor:ew-resize;':'top:0;left:50%;transform:translateX(-50%);height:24px;width:80px;cursor:ns-resize;'}`;
  // Inner visible pill
  const pill = document.createElement('div');
  pill.style.cssText = `position:absolute;background:rgba(255,255,255,0.2);border-radius:4px;${isLand?'right:0;top:50%;transform:translateY(-50%);width:4px;height:36px;':'top:0;left:50%;transform:translateX(-50%);height:4px;width:44px;'}`;
  h.appendChild(pill);
  h.onmouseenter = () => { pill.style.background='rgba(99,153,255,0.6)'; pill.style.transform=isLand?'translateY(-50%) scaleX(1.5)':'translateX(-50%) scaleY(1.5)'; };
  h.onmouseleave = () => { if(!resizing){ pill.style.background='rgba(255,255,255,0.2)'; pill.style.transform=isLand?'translateY(-50%)':'translateX(-50%)'; } };
  let s0, sz0;
  const start = (x,y) => {
    resizing=true; s0=isLand?x:y; sz0=isLand?p.offsetWidth:p.offsetHeight;
    // bottom reset mat karo — quicktools ke upar hi rahega
  };
  const move = (x,y) => {
    if(!resizing) return;
    if(isLand) {
...
function buildPanel() {
  ...
  destroyViewportListener();
  addrEditMode = false;
  // Dimensions update karo — stale state se resize bug hota tha
  _lastW = window.innerWidth;
  _lastH = window.innerHeight;
  const isLand = window.innerWidth > window.innerHeight;
  const vTop   = getViewportTop();
  const vH     = getViewportHeight();
  const panelH = Math.round(window.innerHeight * 0.30);

  panel = document.createElement('div');
  panel.id = 'iw-panel';
  panel.style.cssText = `
    position:fixed;background:#18181b;z-index:50;
    display:flex;flex-direction:column;
    ${isLand
      ? `right:0;top:${vTop}px;width:46%;height:${vH}px;border-left:1px solid rgba(255,255,255,0.08);box-shadow:-4px 0 32px rgba(0,0,0,0.6);`
      : `left:0;bottom:0;width:100%;height:${panelH}px;border-top:1px solid rgba(255,255,255,0.08);border-radius:20px 20px 0 0;box-shadow:0 -8px 40px rgba(0,0,0,0.5);transition:bottom 0.15s;`
    }
  `;

  const hd = document.createElement('div');
  hd.style.cssText = `display:flex;align-items:center;padding:0 10px;background:rgba(30,30,35,0.98);gap:8px;flex-shrink:0;height:54px;border-bottom:1px solid rgba(255,255,255,0.06);${!isLand?'border-radius:16px 16px 0 0;':''}`;

  // W Logo — favicon milne pe replace ho jaayega
  const ico = document.createElement('div');
  ico.id = 'iw-logo';
  ico.style.cssText = `width:40px;height:40px;background:linear-gradient(135deg,#2563eb,#1d4ed8);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:900;color:#fff;flex-shrink:0;box-shadow:0 2px 8px rgba(255,255,255,0.1);overflow:hidden;`;
  ico.textContent = 'W';

  // Address bar wrap
  const addrWrap = document.createElement('div');
  addrWrap.style.cssText = `flex:1;display:flex;align-items:center;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:0 10px;height:34px;min-width:0;`;
  ...
  addrText.style.cssText = `flex:1;background:none;border:none;color:#e5e5ea;font-size:13px;font-weight:500;outline:none;text-align:center;font-family:-apple-system,sans-serif;cursor:pointer;min-width:0;`;
  ...
  updateFab();
  // DOM settle hone ke baad position fix karo — rAF x2
  requestAnimationFrame(() => requestAnimationFrame(() => adjustPanelBottom()));
  loadPreview();
}
...
function createFab() {
  ...
  `;
  fab.textContent = 'W';
  fab.title = 'InstantLive Server LivePreviewer (Ctrl+Shift+W)';
...
function updateFab() {
  ...
  fab.style.background = isVisible
    ? 'linear-gradient(135deg,#1d4ed8,#1e1e1e)'
...
acode.setPluginInit(PLUGIN_ID, () => {
  try {
    ...
    if (commands && commands.addCommand) {
      commands.addCommand({
        name:'iw-preview',
        description:'Toggle InstantLive Server Preview',
  ...
  window.toast && window.toast('InstantLive Server LivePreviewer ready!', 2500);
});
