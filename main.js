...
// Port — user saved port ya default 8080
function getSavedPort() {
  try { return localStorage.getItem('iw_port') || '8080'; } catch(e) { return '8080'; }
}
function savePort(p) {
...
function showError() {
  ...
  d.innerHTML = `
    ...
    ${getSavedPort() !== '8080' ? '<button id="iw-reset-port" style="background:transparent;border:none;color:#475569;font-size:11px;margin-top:8px;cursor:pointer;text-decoration:underline;">Reset to default port (8080)</button>' : ''}
  `;
  ...
  if (resetBtn) resetBtn.onclick = () => {
    savePort('8080');
    const addr = document.getElementById('iw-addr');
    if (addr) addr.value = 'localhost:8080';
    d.remove();
    loadPreview();
  };
}
...
function loadPreview() {
  ...
  fetch(getPreviewUrl()+'/__iw_ping?t='+Date.now(), {mode:'no-cors',cache:'no-store'})
    .then(() => {
      ...
      const isInstantWeb = getSavedPort() === '8080';
